...
public class Matrix extends NMatrix {
   ...
   @*/
    public /*@ pure @*/ Column getColumn(int index) {
        return new Column(_array[index]);
    }
    ...
    public void add(Matrix other) {
        ...
        for(int column = 0; column<columns; column++) {
            for (int row= 0; row<rows; row++) {
                _array[column][row] = _array[column][row] + other.elementAt(row,column);
            }
        }    
    }

    public void addSlow(Matrix other) {
        int rows = getNbRows();
        int columns = getNbColumns();
        for (int row= 0; row<rows; row++) {
            for(int column = 0; column<columns; column++) {
        ...
        for (int column=0; column <otherColumns; column++) {
            for (int row=0; row <rows; row++) {
                ...
                for(int k=1; k <= columns; k++) {
                    ...
                    element+=elementAt(row,k) * other.elementAt(k,column);
                }
                ...
                //	element += terms[k-1];
                //}
                newMatrix.setElementAt(row,column,element);
            }
        }
        return newMatrix;
    }
    ...
    public /*@ pure @*/ Matrix times(double factor) {
        ...
        for (int column=0; column <getNbColumns(); column++) {
            for (int row=0; row <getNbRows(); row++) {
                newMatrix.setElementAt(row,column,elementAt(row,column)*factor);
            }
        }
        return newMatrix;
    }  
   ...
   @*/
    public void multiply(double factor) {
        for (int column=0; column <getNbColumns(); column++) {
            for (int row=0; row < getNbRows(); row++) {
                _array[column][row] = _array[column][row]*factor;
            }
        }
    }  
   ...
   @*/
    public void divide(double factor) {
        for (int column=0; column <getNbColumns(); column++) {
            for (int row=0; row <getNbRows(); row++) {
                _array[column-1][row-1] = _array[column-1][row-1]/factor;
            }
        }
    }  
   ...
   @*/
    ...
    public void leftGivens(double c, double s, int i, int k) {
        //rows 0...i-1, i+1 ... k-1, k+1 ... n remain untouched.
        Row row_i = getRow(i);
        Row row_k = getRow(k);
        setRow(i,row_i.times(c).minus(row_k.times(s)).getRow(0));
        setRow(k,row_i.times(s).plus(row_k.times(c)).getRow(0));
    }
    ...
    public void rightGivens(double c, double s, int i, int k) {
        ...
        setColumn(i,column_i.times(c).minus(column_k.times(s)).getColumn(0));
        setColumn(k,column_i.times(s).plus(column_k.times(c)).getColumn(0));
    }
    ...
    public /*@ pure @*/ Matrix subMatrix(int x1, int y1, int x2, int y2) {
        ...
        for (int column = y1; column <= y2; column++) {
            for (int row = x1; row <= x2; row++) {
                result.setElementAt(row - x1 , column - y1, elementAt(row, column));
            }
        }
        return result;
    }
   ...
   @ public behavior
    ...
    public void setSubMatrix(int row, int column, Matrix matrix) {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < columns; j++) {
                setElementAt(i + row, j + column, matrix.elementAt(i, j));
            }
        }
    }
   ...
   @*/
    public /*@ pure @*/ Object clone() {
        ...
        for(int column=0; column<getNbColumns(); column++) {
            for (int row=0; row<getNbRows(); row++) {
                newMatrix.setElementAt(row,column,elementAt(row,column));
            }
        }
        return newMatrix;
    }  
   ...
   @*/
    public /*@ pure @*/ boolean equals(Object other) {
        ...
        for(int j=0; j<getNbColumns(); j++) {
            for (int i=0; i<getNbRows(); i++) {
                if(_array[j][i] != ((Matrix)other).elementAt(i,j)) {
                    return false;
                }
            }
        }
